...
class AimMove {
 ...
 public:
    ...
    vector<int> GetTargets(int goal_, int width, vector<int>& positions) {
        ...
        bfs_queue.push(m_p(goal_, 0));
        vector<int> surface;
        unordered_set<int> used;
        while (!bfs_queue.empty()) {
            auto top = bfs_queue.front();
            used.insert(top.first);
            bfs_queue.pop();
            if (top.second >= width)
                surface.push_back(top.first);
            if (top.second==width+1) {
                continue;
            }
            for (auto i:graph[top.first]) {
                if (!(used.count(i))) {
                    bfs_queue.push(m_p(i, top.second+1));
                }
            }
        }
        int surface_length = surface.size();
        vector<int> heruestics(surface_length, 0);
        goals.push_back(goal_);
        positions.push_back(goal_);
        int max_heruestic = -1;
        int max_heruestic_val = -1;
        for (auto i =0; i<heruestics.size();++i) {
            
            heruestics[i]=0;
            auto j = positions[0];
            heruestics[i]+=Heruestic(j,surface[i]);
            if (heruestics[i]>max_heruestic_val) {
                max_heruestic_val = heruestics[i];
                max_heruestic = i;
            }
        }
        for (int i =1; i<hunters; ++i) {
            goals.push_back(surface[max_heruestic]);
            positions.push_back(surface[max_heruestic]);
            for (auto j =0; j<heruestics.size();++j) {
                heruestics[j]+=Heruestic(surface[j],*positions.rend())+Heruestic(positions[i],surface[j]);
                if (heruestics[i]>max_heruestic_val) {
                    max_heruestic_val = heruestics[j];
                    max_heruestic = j;
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    while(true) {
        ...
        for (int i=0; i<agent_count; ++i)
            next_path_length = min(agents[i].GetPathSize()/10+1,next_path_length);
