...
class AimMove {
 ...
 public:
    ...
    vector<int> GetTargets(int goal_, int width, vector<int>& positions) {
        ...
        multiset<pair<int,pair<int, pair<int, int>>>> closest;
        for (int i = 1; i< hunters; ++i) {
            for (int j = 1; j< hunters; ++j) {
                // cout<<":"<<i<<' '<<j<<endl;
                closest.insert(m_p(Heruestic(positions[i], goals[j]), m_p(i,m_p(goals[j], j))));
            }
        }
        fill(goals.begin()+1, goals.end(), -1);
        set<pair<int, int>> used_aims;
        // cout<<endl<<closest.size()<<endl;
        while(used_aims.size()!=hunters-1) {
            auto top=*closest.begin();
            // cout<<closest.size()<<' '<<top.second.first<< ' '<<top.second.second<<endl;
            closest.erase(closest.begin());
            if (goals[top.second.first]==-1 and used_aims.count(top.second.second)==0) {
                goals[top.second.first]=top.second.second.first;
                used_aims.insert(top.second.second);
            }
        }
        return goals;
    }
